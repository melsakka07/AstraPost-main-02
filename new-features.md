Please analyze the pages and establish a step-by-step plan to improve the UI and UX. Do not implement all changes at once. Instead, proceed through the plan sequentially and advise on the next step once each phase is complete.

Follow best practices to avoid double scrollbars on the right, ensuring the user can scroll the entire page rather than just one section. Additionally, ensure the footer is fully visible and not truncated from the left.

Once each phase of changes is finished, perform thorough testing to ensure there are no issues with the UI or UX before suggesting the next phase.
