...
export const MILESTONES = {
  ...
  TEN_POSTS: { id: "10_posts", title: "Consistent Creator", description: "Published 10 posts", icon: "✍️" },
  HUNDRED_FOLLOWERS: { id: "100_followers", title: "Growing Audience", description: "Reached 100 followers", icon: "📈" },
  TEN_DAY_STREAK: { id: "10_day_streak", title: "On Fire", description: "Posted for 10 consecutive days", icon: "🔥" },
} as const;

export async function checkMilestone(userId: string, event: "post_published" | "analytics_refresh") {
  // 1. Check Post Milestones
  if (event === "post_published") {
    const postCountRes = await db.select({ count: count() }).from(posts).where(
...
async function calculateStreak(userId: string): Promise<number> {
  ...
  const recentPosts = await db.select({ publishedAt: posts.publishedAt })
    .from(posts)
    .where(and(
