...
export class LinkedInApiService implements SocialApiService {
            ...
            if (response.ok) {
                ...
                await db.update(linkedinAccounts)
                    .set({
                        ...
                        refreshTokenEnc: data.refresh_token ? encryptToken(data.refresh_token) : account.refreshTokenEnc,
  ...
  async getUser(): Promise<SocialUserInfo> {
    const response = await fetch(`${LINKEDIN_API_URL}/me?projection=(id,localizedFirstName,localizedLastName,profilePicture(displayImage~:playableStreams))`, {
