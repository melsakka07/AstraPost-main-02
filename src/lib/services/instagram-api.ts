...
export class InstagramApiService implements SocialApiService {
  ...
  async getUser(): Promise<SocialUserInfo> {
    ...
    const response = await fetch(
      `${GRAPH_API_URL}/${this.instagramAccountId}?fields=${fields}&access_token=${this.accessToken}`
    );
  ...
  async getFollowerCount(): Promise<number> {
    const response = await fetch(
      `${GRAPH_API_URL}/${this.instagramAccountId}?fields=followers_count&access_token=${this.accessToken}`
    );
  ...
  async post(content: SocialPostContent): Promise<{ id: string; url?: string }> {
    ...
    if (mediaItem.fileType === "image") {
      ...
      const createRes = await fetch(
        `${GRAPH_API_URL}/${this.instagramAccountId}/media?${params.toString()}`,
        {
          method: "POST",
        }
      );
      ...
      if (!createRes.ok) {
        ...
        throw new Error(
          `Instagram Media Creation Failed: ${err.error?.message || "Unknown error"}`
        );
      }
      ...
      if (!createRes.ok) {
        const err = await createRes.json();
        throw new Error(
          `Instagram Video Creation Failed: ${err.error?.message || "Unknown error"}`
        );
      }
    ...
    const publishRes = await fetch(
      `${GRAPH_API_URL}/${this.instagramAccountId}/media_publish?${publishParams.toString()}`,
      {
        method: "POST",
      }
    );
    ...
    const permalinkRes = await fetch(
      `${GRAPH_API_URL}/${publishData.id}?fields=permalink&access_token=${this.accessToken}`
    );
  ...
  private async waitForMediaProcessing(containerId: string): Promise<void> {
    ...
    while (attempts < maxAttempts) {
      const res = await fetch(
        `${GRAPH_API_URL}/${containerId}?fields=status_code,status&access_token=${this.accessToken}`
      );
