...
| `OPENROUTER_MODEL_YOUTUBE_TO_THREAD` | optional                                                 | Dedicated model for YouTube-to-thread generation (async BullMQ worker). Falls back to `OPENROUTER_MODEL`.                                                                                                                           |

### Image Models (Replicate) — `src/lib/env.ts:62-67`, mapping in `src/lib/services/ai-image.ts:478-488`
...
| (agentic custom)  | `REPLICATE_MODEL_AGENTIC`  | optional | varies                                |

> `REPLICATE_MODEL_AGENTIC` is a custom model override for agentic pipeline image generation. When set, it replaces `REPLICATE_MODEL_FAST` for the agentic image step. Falls back to `REPLICATE_MODEL_FAST` when unset.
...
| 9b  | `POST /api/ai/pdf-to-thread/generate`  | `generateObject` | `OPENROUTER_MODEL_PDF_TO_THREAD` → `OPENROUTER_MODEL` | **Pro+** (`checkPdfToThreadAccessDetailed`, quotaWeight: 5) | **5**        | Sync thread generation for PDFs ≤30,000 chars. Uses `buildSummarizePrompt({ variant: "report" })` + `pdf_to_thread:v1` prompt version. `JAILBREAK_GUARD`, PII redaction, moderation check on output. Returns `{ tweets, title, sourceLanguage }`. Writes `aiGenerations` row via `recordAiUsage()`. |
...
| 9f  | `GET /api/ai/pdf-to-thread/history`    | none             | n/a                                                   | Pro+ (ownership-checked)                                    | n/a          | Returns last 5 ready PDF thread jobs (fileName, pageCount, title, completedAt). Auth via `getTeamContext()`.                                                                                                                                                                                        |

### A.3 YouTube-to-Thread (Pro+ gated, always async)

| #   | Endpoint                                     | SDK  | Model | Eligible                                                                                                         | Quota Weight | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --- | -------------------------------------------- | ---- | ----- | ---------------------------------------------------------------------------------------------------------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 9g  | `POST /api/ai/youtube-to-thread`             | none | n/a   | **Pro+** (`checkYoutubeToThreadAccessDetailed`, quotaWeight: 5)                                                  | **5**        | Validates YouTube URL via `getVideoInfoHttp()` (HTTP-based innertube API with oEmbed fallback), creates `youtubeThreadJobs` row, enqueues to `youtubeThreadQueue`. Supports `previewOnly` mode (skips quota, returns `{ status: "validated", videoTitle, durationSeconds, thumbnailUrl, durationVerified }`). Normal mode returns `{ jobId, status: "queued", videoTitle, durationSeconds, thumbnailUrl }`. `aiPreamble` handles auth + plan gate + quota. Enqueues AFTER `db.transaction()` commits. |
| 9h  | `GET /api/ai/youtube-to-thread/[jobId]`      | none | n/a   | Pro+ (ownership-checked)                                                                                         | n/a          | Poll job status/result. Returns `{ status, videoTitle, durationSeconds, threadResult, error, createdAt, queuedAt, startedAt, completedAt }`. Auth via `getTeamContext()`.                                                                                                                                                                                                                                                                                                                             |
| 9i  | `DELETE /api/ai/youtube-to-thread/[jobId]`   | none | n/a   | Pro+ (ownership-checked)                                                                                         | n/a          | Cancel a queued/processing job. Sets status `"failed"` with error `"user_cancelled"`. Best-effort removes from BullMQ. Auth via `getTeamContext()`.                                                                                                                                                                                                                                                                                                                                                   |
| 9j  | `GET /api/ai/youtube-to-thread/history`      | none | n/a   | Pro+ (ownership-checked)                                                                                         | n/a          | Returns last 5 ready YouTube thread jobs (thumbnailUrl, title, completedAt). Auth via `getTeamContext()`.                                                                                                                                                                                                                                                                                                                                                                                             |
| 9k  | `GET /api/ai/youtube-to-thread/capabilities` | none | n/a   | All plans (session)                                                                                              | n/a          | Returns available transcription providers: `{ providers: { deepgram: boolean, whisper: boolean } }`. Deepgram requires `YOUTUBE_DEEPGRAM_API_KEY`, Whisper requires `OPENAI_API_KEY`. Auth via `getTeamContext()`.                                                                                                                                                                                                                                                                                    |
| 9l  | `POST /api/ai/thread-first-image`            | none | n/a   | **Pro+** (`checkPdfToThreadAccessDetailed` / `checkYoutubeToThreadAccessDetailed` + `checkAiImageQuotaDetailed`) | n/a (image)  | Generates an editorial 16:9 image for the first tweet of a PDF/YouTube thread. Called client-side after thread is ready. Auth via `getTeamContext()` + role check. Rate limited (`ai_image` bucket). Image quota gate (`checkAiImageQuotaDetailed`). Delegates to `generateAgenticImage()` which records usage internally. Model: `REPLICATE_MODEL_AGENTIC` → `REPLICATE_MODEL_FAST`. Returns `{ url, width: 1344, height: 768 }`.                                                                    |

**Worker-side AI** (`youtubeThreadProcessor` in `src/lib/queue/processors.ts`):
| Step | SDK Primitive | Model | Notes |
|---|---|---|---|
| Transcription | Deepgram REST API / OpenAI Whisper | `YOUTUBE_DEEPGRAM_API_KEY` or `OPENAI_API_KEY` | Downloads audio via yt-dlp (with proxy, cookies, iOS UA headers). Falls back to title-only thread generation when audio download is blocked (all production data-center IP cases). |
| Thread Generation | `generateObject` | `OPENROUTER_MODEL_YOUTUBE_TO_THREAD` → `OPENROUTER_MODEL` | Generates structured thread from transcript (or video title as fallback) with JAILBREAK_GUARD + moderation check. |
...
17. **New env var `REPLICATE_MODEL_AGENTIC`** — optional custom Replicate model for agentic pipeline image generation. Falls back to `REPLICATE_MODEL_FAST`. Added 2026-05-08.
18. **New PDF-to-Thread history endpoint** — `GET /api/ai/pdf-to-thread/history` returns last 5 ready jobs. Added 2026-05-07, was missing from earlier audit versions.
19. **YouTube worker now has title-only fallback mode** — when audio download is IP-blocked (all data-center IP cases without proxy/cookies), the worker generates threads from the video title alone. Also supports `YOUTUBE_PROXY_URL` proxy and `YOUTUBE_COOKIES_BASE64` cookie env vars for bypassing bot detection.
