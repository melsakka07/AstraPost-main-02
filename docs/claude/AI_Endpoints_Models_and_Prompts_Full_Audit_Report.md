...
| #   | Endpoint                                   | SDK  | Model | Eligible                                                                                                         | Quota Weight | Notes                                                                                                                                                                                                                                                                                                                                                                                           |
| --- | ------------------------------------------ | ---- | ----- | ---------------------------------------------------------------------------------------------------------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 9f  | `POST /api/ai/youtube-to-thread`           | none | n/a   | **Pro+** (`checkYoutubeToThreadAccessDetailed`, quotaWeight: 5)                                                  | **5**        | Validates YouTube URL via yt-dlp, creates `youtubeThreadJobs` row, enqueues to `youtubeThreadQueue`. Quota consumed at enqueue time. Returns `{ jobId, status: "queued", videoTitle, durationSeconds }`. `aiPreamble` handles auth + plan gate + quota. Enqueues AFTER `db.transaction()` commits.                                                                                              |
| 9g  | `GET /api/ai/youtube-to-thread/[jobId]`    | none | n/a   | Pro+ (ownership-checked)                                                                                         | n/a          | Poll job status/result. Returns `{ status, videoTitle, durationSeconds, threadResult, error, createdAt, queuedAt, startedAt, completedAt }`. Auth via `getTeamContext()`.                                                                                                                                                                                                                       |
| 9h  | `DELETE /api/ai/youtube-to-thread/[jobId]` | none | n/a   | Pro+ (ownership-checked)                                                                                         | n/a          | Cancel a queued/processing job. Sets status `"failed"` with error `"user_cancelled"`. Best-effort removes from BullMQ. Auth via `getTeamContext()`.                                                                                                                                                                                                                                             |
| 9i  | `POST /api/ai/thread-first-image`          | none | n/a   | **Pro+** (`checkPdfToThreadAccessDetailed` / `checkYoutubeToThreadAccessDetailed` + `checkAiImageQuotaDetailed`) | n/a (image)  | Generates an editorial 16:9 image for the first tweet of a PDF/YouTube thread. Called client-side after thread is ready. Auth via `getTeamContext()` + role check. Rate limited (`ai_image` bucket). Image quota gate (`checkAiImageQuotaDetailed`). Delegates to `generateAgenticImage()` which records usage internally. Model: `nano-banana-2`. Returns `{ url, width: 1344, height: 768 }`. |

**Worker-side AI** (`youtubeThreadProcessor` in `src/lib/queue/processors.ts`):
| Step | SDK Primitive | Model | Notes |
|---|---|---|---|
| Transcription | Deepgram REST API / OpenAI Whisper | `YOUTUBE_DEEPGRAM_API_KEY` or `OPENAI_API_KEY` | Downloads audio via yt-dlp, sends to transcription provider. |
| Thread Generation | `generateObject` | `OPENROUTER_MODEL_YOUTUBE_TO_THREAD` → `OPENROUTER_MODEL` | Generates structured thread with JAILBREAK_GUARD + moderation check. |
