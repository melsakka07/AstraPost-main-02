...
- [ ] **`pnpm lint && pnpm typecheck`** — passes cleanly with zero errors
...
Implement all phases in order. After each phase, run `pnpm lint && pnpm typecheck` to verify correctness before moving on.
