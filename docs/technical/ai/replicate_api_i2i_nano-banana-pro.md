...
Integration Notes (Astra Art AI Studio)

- Endpoint: `/api/generations` with `generationType: "image_to_image"`, `modelId: "google/nano-banana-pro"`.
- Input mapping: `inputPrompt` → `prompt`; `inputImageUrl[]` or avatar base → `image_input`; `parameters.aspect_ratio`/`resolution`/`output_format`/`safety_filter_level` pass-through.
- Output: image URI stored in `generation_assets`.
- Credits: 1 per job.
